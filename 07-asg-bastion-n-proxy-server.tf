...
resource "aws_launch_template" "bastion-lt" {
  ...
  tag_specifications {
    ...
    tags = merge({ "Name" : "MC-${workspace}-Bastion-LaunchTemplate" }, local.tags)
  }
...
resource "aws_autoscaling_group" "bastion-asg" {
  ...
  tag {
    key                 = "Name"
    value               = "MC-${workspace}-Bastion-ASG"
...
resource "aws_launch_template" "proxy-server-lt" {
  ...
  tag_specifications {
    ...
    tags = merge({ "Name" : "MC-${workspace}-ProxyServer-LaunchTemplate" }, local.tags)
  }
...
resource "aws_autoscaling_group" "proxy-server-asg" {
  ...
  tag {
    key                 = "Name"
    value               = "MC-${workspace}-ProxyServer-ASG"
    propagate_at_launch = true
  }
